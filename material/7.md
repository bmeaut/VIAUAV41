...
When we define a function we list the parameters that it takes, including the types of its parameters. This, however, is not quite enough to describe the *contract* of a function. We might also want to specify how and when the function should be called. We might place a restriction on the current state of the application, or the values of the arguments that are being passed in. To look at how Kotlin helps us clearly state these expectations, we'll use a simple `Reader` interface:
...
> What we're doing here falls in line with another item of *Effective Java*: *Item 49: Check parameters for validity*.

[`check`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/check.html) is meant to ensure that our application is in some expected state. What we want to check in this case is that our `inputStream` is non-null when `readBytes` is called:
...
Notice how having these checks and requirements at the very top of our method makes them easy to read if someone navigates to the source code. They are also clearly separated from the actual implementation of the method.
...
Worry not, we're just looking at another example of [extension oriented design](./4.md#extension-oriented-design) here. The class contains only the core functionality that it needs (e.g. `get`, `length`, or `plus`), presenting a minimal API, and everything else is implemented on top of this as extensions.
...
A handy language feature that we didn't cover yet is [*multiline strings*](https://kotlinlang.org/docs/strings.html#multiline-strings). A multiline string is bounded by `"""` (triple quotation marks) on each end, and it can contain nearly any character. This includes those that usually have special meaning inside a `String`, such as `\` and even newlines.
...
Kotlin simplifies opening a file for reading or writing with extensions. You can use these to open a stream for a file, or to access even higher level APIs, such as various readers and writers:
...
If you use these APIs, it's your responsibility to free these resources when you're done using them. Remember that you have the [`use`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/use.html) function in the Standard Library that can do this for you safely. This function returns whatever the lambda passed to it returns, and closes the opened resource:
...
> This kind of data processing can be done in just a regular Kotlin JVM program, but it's also an excellent use case for a [Kotlin Notebook](https://kotlinlang.org/docs/kotlin-notebook-overview.html), which is one of the many tools around Kotlin for working with data. You can combine Notebooks with the [Kandy](https://github.com/Kotlin/kandy) library to visualize the data with proper charts, and they also integrate nicely with [Kotlin DataFrame](https://github.com/Kotlin/dataframe) for actually processing the data (though we'll stick to the Standard Library for now). For a brief introduction to these various tools, watch [this official video](https://www.youtube.com/watch?v=m4Cqz2_P9rI).

The data set consists of three files, containing stats from the years 2014, 2015, and 2016. The files are [CSVs](https://en.wikipedia.org/wiki/Comma-separated_values), and they contain data like this:
...
The only conversion not provided to us is the Standard Library is the `toLocalDateTime` function. We'll implement this ourselves. Dates in our data are in the ISO 8601 local date-time format, for example: `2014-01-18T15:52:00`. We can easily obtain these number values with the following `String` operations:
...
> CTA is the [Chicago Transit Authority](https://www.transitchicago.com/).
...
Note that measurements like `measureNanoTime` and similar (such as `System.currentTimeMillis()`) are not a great way of benchmarking code. There are dedicated tools for this purpose which you should use instead, for example, the [kotlinx-benchmark](https://github.com/Kotlin/kotlinx-benchmark) library. If you're running code on Android, take a look [at this documentation](https://developer.android.com/studio/profile/benchmark).
...
A grouping is very similar to a `Map`, but like a `Sequence`, creating it doesn't trigger any processing yet. We need to call a further terminating operation on it. One of these is [`eachCount`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/each-count.html), which is exactly what we need:
...
- Kotlin for Data Analysis
  - [Kotlin Notebook](https://kotlinlang.org/docs/get-started-with-kotlin-notebooks.html)
