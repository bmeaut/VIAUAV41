...
Let's create some specific snacks:
...
Let's see if we can use a `Box<Pretzel>` where a `Box<Snack>` is expected. We'll create a function that takes a `Box<Snack>` as a parameter, and uses its functionality. It first empties the entire box, and then inserts a new `Snack` in it.
...
However, then we want to insert a `Donut()` into the `Box<Snack>` (and we could attempt this with any other `Snack` instance as well). This is a problem, as a `Box<Pretzel>` that only accepts `Pretzel` instances. *This means that a `Box<Pretzel>` is not a `Box<Snack>`.*

![Subtyping error for Box](images/10_broken_box.png)

We've seen that a `Box<Pretzel>` worked fine in the place of a `Box<Snack>` in certain ways, and not others. It was fine as long as we only took generic values *out* of it. If we just remove the problematic method that takes values of `T` as a parameter, our `Box<Pretzel>` would be safe to use as a `Box<Snack>`.
...
This behavior, marked by the `out` keyword, is called *covariance*. A generic type `Generic<T>` is covariant on its type parameter if `X` being a subtype of `Y` means that a `Generic<X>` will be a subtype of a `Generic<Y>`. The **subtyping direction is the same** for the parameters and the generic types that use them. They vary *together*, they are *co*variant.

![Covariance](images/10_covariance.png)

#### Contravariance

What about putting snacks into a `Box`? We've seen that a `Box<Pretzel>` can't work as a `Box<Snack>` for these purposes, because a `Box<Snack>` has to accept any kind of `Snack`. However, we can spot an inverse relationship here: a `Box<Snack>` could work as a `Box<Pretzel>` in this case, as it can accept `Pretzel` instances!
...
This behavior, marked by the `in` keyword, is called *contravariance*. A generic type `Generic<T>` is contravariant on its type parameter if `X` being a subtype of `Y` means that a `Generic<Y>` will be a subtype of a `Generic<X>`. The **subtyping direction is reversed** between the type arguments and the generic types that use them. They vary in the *opposite* direction, they are *contra*variant.

![Covariance and contravariance](images/10_co_and_contravariance.png)

#### Invariance

With these new interfaces in place, we can easily reintroduce the `Box` type as the combination of these two interfaces:
