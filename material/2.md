...
This code won't compile, and produces the following error for both uninitialized properties:

`e: Property must be initialized or be abstract`

Constructing an instance of `Person` like this wouldn't be safe: what would happen when someone reads the value of `name` or `age`?

*This behaviour of Kotlin forces you to explicitly initialize every value in one way or another, and guarantees that your properties won't have implicit values stored in them.* Whenever you read a property, you'll get a value out of it that *you* have put there, intentionally.
...
> ```
>
> This class has an implicit constructor with no parameters. When you call its constructor with `new Person()`, its fields will be initialized to implicit default values: `null` and `0`, respectively. In general, primitive types are initialized to some resemblance of `0`, while reference types are initialized to `null`.
...
These different types of initializations are performed from top to bottom, in order. In the example, `model` and `miles` would be initialized first, and then finally `year` would get its value. *Any parameters that the primary constructor takes may be used for these initializations*.
