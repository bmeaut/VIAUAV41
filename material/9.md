...
Thanks to Kotlin's strong typing, we rely on its type system to write safe code whenever we're using the language - probably more than you realize. Now it's time to examine this type system, get familiar with its special types, and learn about its mechanisms in detail.
...
It's important to note that types are **not** equivalent to classes. We create new types every time we declare a class, interface, object, or typealias (going forward, we'll stick mostly to just classes for simplicity). In fact, in all of these cases, we create multiple new types with these declarations. Let's take a boring, empty class:
...
In fact, this unbounded `T` type parameter introduces infinitely many types. Some of these would be `List<String>`, `List<String>?`, `List<String?>`, `List<String?>?`, `List<List<Int>>`, `List<in String>`, `List<*>`... Just to mention a few. (We'll take a look at what all of these mean in [the next chapter](./10.md).)

### The `Any` type

Like classes, types exist in a hierarchy. We'll put aside nullable types for a moment, and look at the hierarchy of non-nullable types. This looks essentially the same as the hierarchy formed by the respective classes that create these types.

![The Kotlin type hierarchy](./images/9_typical_simplest.png)

The root of Kotlin's type hierarchy is the `Any` type (just like the [`Any`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-any/) class is the root of its class hierarchy). This type defines basic functions such as `equals`, `hashCode`, and `toString`, which are available on all instances in Kotlin, as they all have `Any` as their supertype.

Classes with no explicit superclass inherit directly from the `Any` class, and therefore the non-nullable types produced by these classes are direct subtypes of the `Any` type. An example of this is the `Hello` class we've defined earlier. As Kotlin doesn't distinguish between primitives and wrappers, the basic types (`Int`, `Double`, `Boolean`, etc.) are also all subtypes of `Any`.
...
This requirement is twofold. On one hand, it's a technical requirement, which the type system can verify for us. For a concrete example, we know that if we create a `SavingsAccount` class and then inherit from it with an `AccessibleSavingsAccount` class, then the compiler lets us pass an `AccessibleSavingsAccount` to a function that expects to operate on a `SavingsAccount`:
...
We tend to summarize why this works very simply by saying that an `AccessibleSavingsAccount` *is a* `SavingsAccount`. 

However, inheriting from the base class with the syntax `: SavingsAccount()` is just part of what makes an `AccessibleSavingsAccount` a `SavingsAccount`.

The other requirement for this subtyping to be correct is that the `AccessibleSavingsAccount` class must *fulfill the contract of a `SavingsAccount`*. If we have a reference of type `SavingsAccount` which points to an `AccessibleSavingsAccount` instance, we should be able to call methods on this object and get behaviour consistent with what we expect from the base `SavingsAccount` type. This is one of the SOLID principles, namely, the [Liskov substitution principle](https://en.wikipedia.org/wiki/Liskov_substitution_principle).

> This second requirement can technically be broken, and it's rather easy to break it. Doing so is a code smell called *refused bequest* - a class inheriting from another class, without upholding its contract. This is usually a misguided effort to reuse code from the superclass. You'll find this discussed in [Effective Java](https://www.oreilly.com/library/view/effective-java-3rd/9780134686097/) in detail, in *Item 18: Favor composition over inheritance*.

Let's have `SavingsAccount` contain a method that lets us deposit a certain amount of money, and returns the updated balance. For any method, its *contract* consists of the set of input values it can accept, plus the promises it makes about its return values. For this one, we could say that its parameter `amount` always has to be non-negative `Int`, and what it promises about its return value is that it'll be some kind of `Number`.
...
open class SavingsAccount {
    open fun deposit(amount: Int): Number {
        require(amount >= 0)
        // Compute and return the updated balance...
    }
}
```
...
> As a reminder, the unsigned types of Kotlin [should not be used for this purpose](https://kotlinlang.org/docs/unsigned-integer-types.html#non-goals).
...
What happens when we override a method like this in a subtype? We need to match the contract it originally had, as someone might use our `AccessibleSavingsAccount` as a `SavingsAccount` at some point.

The easy way to remember the Liskov principle is with this short phrase: **expect no more, provide no less**.

**Expect no more.** We can't define stricter expectations for inputs than our superclass did, as we have to be able to process everything that the superclass is able to process. We can usually make our input requirements looser, if we want: for our concrete example, we'll allow negative values in the subclass.

> This of course assumes that we don't consider an exception being thrown for negative values part of the contract of `SavingsAccount`, which is up for debate.
...
Let's take a really simple implementation of the subclass, which just works with an `Int` balance internally:
...
As you can see, we are even allowed to change signature of the method we're overriding in the subclass to the more concrete return type (changed from `Number` to `Int` above).
...
Now, let's see how nullable types fit into the picture of our type system. The relation of a given non-nullable type and its nullable counterpart is fairly simple to determine. The non-nullable type is a more specific type, as it can only accept a subset of the values that the nullable type can (all of them except for `null`).
...
With our newly developed knowledge of the Kotlin type hierarchy, we can get more familiar with a feature that we've already been using for a while - the Elvis operator.

This operator is most often used to handle cases where a value is `null` and we have to use a substitute value in its place. If the value on its left happens to be `null`, it will return the value on its right.
...
This is the straightforward case, and `definitelyString` will simply have the type `String` here, as expected.
...
Let's answer these questions by looking at some examples. Here's a small, simple type hierarchy we'll use for these:

![A hierarchy of animals and plants](./images/9_animals_0.png)

##### Both nullable 

Let's evaluate an Elvis expression between a `Dog?` and a `Horse?` type first. We'll use this notation to describe this task:

![Dog? ?: Horse?](./images/9_animals_1_0.png)

First, we have to find these types in the hierarchy:
...
Again, we'll take the first common supertype, as this worked well before, and we have no reason to doubt it so far. We could take multiple routes from `Dog` to `Animal?` here, however, this doesn't affect the end result.
...
However, if we take a step back, we'll also find that the Elvis operator is simply redundant in this case - the left side can never be `null`! Nevertheless, if we were to still write this down and not heed the IDE's warnings to remove the redundant Elvis operator, we'd get `Animal?` as the return type.
...
Something has gone wrong here. This nullable result of `Animal?` is overly cautious. If the left-hand side of the expression happens to be `null`, we'll be using the right-hand side instead, which will give us a non-nullable `Horse` value. If the left side is not `null`, we get a non-nullable `Dog` as the result.

This means that `Animal` would be a perfectly reasonable result for the expression as well. What went wrong here, how did we get a nullable type when none is needed? Our mistake was to treat the left side as nullable, even though an Elvis expression will never return `null` from that side.

To fix this, we'll adjust our "algorithm": we'll first take the non-nullable version of the type on the left, and the type on the right as-is, and then find the first common supertype of *these* two types.

![Animal correctly highlighted as the result of a Dog? and Horse expression](./images/9_animals_3_3.png)

In our case, this means going from `Dog?` to the non-nullable side of the hierarchy, and then finding the first common supertype of `Dog` with `Horse`, which, as expected, will now be the `Animal` type.

This is the correct way of determining the return type of an Elvis expression. We'll use this idea again later this chapter!

### Unit

`Unit` is a special type in Kotlin that we've already encountered in the very first chapter, but we'll examine it further now.

The documentation describes it as *the type with only one value*. We can also look at its source, where we can see that it's declared as a singleton `object`:
...
The Kotlin compiler allows both of these functions to compile since it understands control flow and sees that neither will ever reach a `return` statement. Of course, both of these *could* just return `Unit`, but this way, we signal to client code that they will never return.
...
For example, the `loopy` function shown above will never return properly, which makes this variable assignment and then the call to the `processData` function completely safe to compile, albeit nonsensical:
...
The specific type of `processData`'s parameter doesn't matter either. It could be any arbitrary type, and `Nothing` could still be passed in safely (as it will never actually be passed in!).
...
> IntelliJ IDEA also picks up invocations of this function the same way that it picks up comments containing "TODO", so you can view them on the same [tool window](https://www.jetbrains.com/help/idea/todo-tool-window.html).

#### Nothing ft. Elvis

We're going to bring two previous topics together now, `Nothing` and the Elvis operator.

Consider what happens when you use an `Elvis` operator with something of type `Nothing` on the right-hand side. If the expression's left side ends up being non-null, that's the result of the expression, and we can continue on with our code. However, if we have to evaluate the "default" right side, execution of whatever function we're in is guaranteed to be halted, as evaluating that side will never terminate normally.
...
Notice that we are using `x` as an `Int` on the third line where we're multiplying it. This code compiles and works correctly, despite the `someParam` parameter having a type of `Int?`. This makes sense intuitively. If we were to run into the right-hand side case of the Elvis expression and call the `exceptional` function, `calculate` would not proceed due to the exception thrown from there, and otherwise we have a non-null `Int` on our hands.
...
There's another expression that can be used here for a different effect - the `return` expression. This lets us stop execution of this function for an invalid argument silently.
...
In all these examples, we created a new variable that's essentially the same as the parameter. This is actually unnecessary, and the following is a common pattern in Kotlin:
...
Why does this still work? Because the Elvis expression (as every other expression) will be evaluated even when its return value is unused, and if the parameter was `null`, it returns from the function. From the next line, `someParam` will be available as an `Int` due to a smart cast.

>  This pattern works with any `Nothing`-returning expression, including `throw` or function calls returning `Nothing`.
...
Why does this work? Both `return` and `throw` take an argument that needs to be evaluated before they're executed. `throw` takes a `Throwable`, and `return` takes whatever the return type of the given function is.

> In this case, that return type is `Unit`, which is applied implicitly. If the function returned an `Int`, the same pattern would still work, as long as we ended it with something like `return 1`.
...
Adding parentheses can clarify what's happening:
...
When introducing `Nothing` as a bottom type, we updated our type hierarchy with two new types, but all we've discussed so far was the non-nullable `Nothing`. Does the nullable type `Nothing?` make any sense, and is it of any use to us?
...
Hopefully going forward this won't scare you, and you'll realize quickly that inference is to blame. You can fix this issue by explicitly typing `x` as a `String?`:
...
The Kotlin type system is perhaps the most important part of the language. It's the source of the safety that pervades the entire language, as it provides safe null handing, well-defined subtyping, and powerful support for advanced generics (which we'll take a look at in the next chapter).
