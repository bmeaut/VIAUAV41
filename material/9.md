...
This requirement is twofold. On one hand, it's a technical requirement, which the type system can verify for us. For a concrete example, we know that if we create a `SavingsAccount` superclass and then inherit from it with an `AccessibleSavingsAccount` class, then the compiler lets us pass an `AccessibleSavingsAccount` to a function that expects to operate on a `SavingsAccount`:
...
What happens when we override a method like this in a subtype? We are required to hold up the contract it originally had, as someone might use our `AccessibleSavingsAccount` as a `SavingsAccount` at some point.
...
As you can see, we are even allowed to change signature of the method we're overriding in the subclass to the more concrete return type (from `Number` to `Int`, above).
