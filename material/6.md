...
To avoid duplication, we can extract the validation logic for these strings into a function. However, since we only need to use this specific logic in one place in our application, this can be a local function, which will only be in scope within the enclosing function:
