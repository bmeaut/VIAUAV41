...
Previosuly, we've covered Kotlin's first-class support for functions and function types. Now, let's dive even further into some of its capabilities related to functions.
...
The business rule is that for any fields that are missing a value, both the title and the corresponding value field should be hidden. Instead of writing this logic over and over for each field, we can extract it into a local function, which will only be in scope within the enclosing function:
...
This function receives a lambda as a parameter, which itself *is an extension on `StringBuilder`*. This is practically the same as a lambda that receives a `StringBuilder` as a parameter, it just takes that reference as its *receiver* instead - either way, it can operate on it. Inside `buildString`, we perform the pattern pointed out earlier:
...
Now, let's make use extension functions, and make our helper function an extension on `Database`, to make the call site even nicer:
...
This means that instead of doing the multiplication going upwards, while unwinding the stack, we're instead performing it on the way down to the bottom. When we reach the bottom, we already have our final value computed. At this point, all the intermediate steps of walking back through the stack are basically useless. Nothing happens in those intermediate functions anymore, they all just return what they've received from a level down. We don't really need the stack here.
...
[`also`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/also.html) is similar to `apply`, as it returns its receiver. It also executes the lambda passed to it, where it passes its receiver in as a parameter (unlike `apply`, which uses a receiver instead). 
...
Like `apply`, this is usually used around object creation, to perform some kind of side effects:
...
[`run`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/run.html) is similar to `let`, it returns the result of the result of the lambda passed to it, performing a mapping operation. However, inside the lamda, it exposes its receiver as a receiver (unlike `let`, which uses a regular function parameter instead).
