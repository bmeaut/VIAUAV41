...
[Why?](https://stackoverflow.com/q/51638557/4465208) Mostly to make sure that whatever you want optimized is truly eligible for the optimization. If you mark a function that's not truly tail recursive with the keyword, you'll get a warning.
...
Let's start with [`let`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/let.html). `let` is defined as an extension on a generic `T` type, which means you can invoke it on any value. It takes a lambda as its parameter, which accepts the same `T` type, and returns some `R` type.
...
Finally, the [`with`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/with.html) function deserves an honourable mention. It's just like `run`, as it places code into the scope of a given object. However, instead of being an extension, it's a regular function that takes the object it operates on as a parameter.
...
> The [real implementation](https://github.com/JetBrains/kotlin/blob/eb6e6b7b526f06ff72b56de3c7949d91f84d50e6/libraries/stdlib/jvm/src/kotlin/io/Closeable.kt#L20-L44) has a lot more code than this - exceptions and Closeables are a complex topic.
