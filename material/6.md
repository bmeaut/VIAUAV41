...
This recursive implementation comes with the downside of requiring callers to pass in an extra `1` as a parameter, which is a bit odd. This can be easily fixed by adding a one-parameter helper function, which clients should use (and the recursive function could then be marked `private`):
