...
It turns out this is an example of code that the built-in decompiler of IntelliJ IDEA isn't able to handle well. If we use another decompiler (such as [jadx](https://github.com/skylot/jadx)), we'll see what's actually going on:
...
`null.INSTANCE` in the previous decompiler output, then, was really just a read to this static field to get the instance, except the decompiler didn't manage to resolve the classname. What we should see there is this:
...
We've seen what happens when a lambda relies on no external values. This isn't necessarily the case though. Lambdas act as [closures](https://en.wikipedia.org/wiki/Closure_(computer_programming)), which means they will capture any variables from outer scopes that are referenced inside them.
...
3. Inside the anonymous class implementing our lambda, the current `String` instance is taken from the `ObjectRef`, concatenation happens, and then the new `String` instance is placed in the `ObjectRef`.
...
Passing around lambdas is simple, convenient, and it allows for powerful abstractions that we'll see great examples of when we discuss collections in [the next chapter](5.md). However, the previous example shows that we pay a price when passing lambdas to functions. Each lambda requires an object allocation, and capturing references can lead to further allocations.
...
With a function as simple as this, the IDE will warn us that the gains from inlining the method won't be significant, as function calls are not too expensive in general. When running on the JVM, the virtual machine can also perform optimizations equivalent to inlining on its own.
...
A slightly more complex use case is when you're passing in a lambda somewhere that you want inlined, but you can't allow non-local returns from it, as it doesn't make sense in the execution context.
