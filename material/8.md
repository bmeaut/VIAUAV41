...
Like many other modern languages, Kotlin supports operator overloading. However, unlike some other languages, it doesn't let you define arbitrary symbols as operators. Instead, it confines you to a predefined set of operators that you may define for each type.
...
> The  [`BigDecimal#sqrt`](https://docs.oracle.com/javase/9/docs/api/java/math/BigDecimal.html#sqrt-java.math.MathContext-) method is only available from Java 9, and you'd have to define your own top level `sqrt` function that wraps it to get the syntax seen above.

> This is **not** a proper implementation of a quadratic equation solver. Write a better one as practice!
