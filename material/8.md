...
The second interface, [`ReadWriteProperty`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.properties/-read-write-property/index.html) can be used for properties that are either read-only (`val`) or read-write (`var`), as it also has a `setValue` method. This method takes a `V` as its last parameter, and returns `Unit`.
